...
class CategoryController {
    ...
    index(req, res, next) {
        ...
        let query = 'SELECT * FROM loai ORDER BY tenL ASC';
        if (sortBy && sortOrder) {
    ...
    create(req, res) {
        ...
        const query = `INSERT INTO loai (idL, tenL, hinhanh) VALUES (?, ?, ?)`;
        const values = [short.generate(), req.body.tenL, req.body.hinhanh];
    ...
    edit(req, res) {
        const id = req.params.id;
        db.query('SELECT * FROM loai WHERE idNH = ?', [id], (error, results) => {
            if (error) {
                console.error('Error executing MySQL query: ', error);
                res.status(500).json({ error });
                return;
